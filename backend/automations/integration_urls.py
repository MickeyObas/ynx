...
from .views import oauth_complete, start_oauth, connection_test, integration_list, integration_detail, trigger_list

urlpatterns = [ 
    path("", integration_list),
    path("<str:integration_id>", integration_detail),
    path("<str:integration_id>/triggers", trigger_list),
    path("oauth/<str:service_name>/callback/", oauth_complete),
